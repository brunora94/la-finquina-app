...
import { Home, Sprout, ClipboardList, Package, Trophy, Wrench } from 'lucide-react';
        ...
        <div className="min-h-screen bg-earth-50 pb-24 md:pb-0 md:pl-24">
            ...
            <nav className="hidden md:flex flex-col fixed left-0 top-0 bottom-0 w-24 bg-white border-r border-nature-100 items-center py-8 z-30">
                ...
                <div className="flex flex-col gap-8 w-full">
                    ...
                    <NavItem icon={Wrench} label="Taller" active={currentPage === 'machinery'} onClick={() => onNavigate('machinery')} />
                    <NavItem icon={Trophy} label="Éxitos" active={currentPage === 'harvests'} onClick={() => onNavigate('harvests')} />
                </div>
            </nav>

            {/* Mobile Bottom Bar (Thumb Friendly) */}
            <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-nature-100 px-2 py-2 pb-6 z-30 flex justify-between shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
                <NavItem icon={Home} label="Inicio" active={currentPage === 'dashboard'} onClick={() => onNavigate('dashboard')} />
                <NavItem icon={Sprout} label="Cultivos" active={currentPage === 'crops'} onClick={() => onNavigate('crops')} />
                <NavItem icon={Wrench} label="Taller" active={currentPage === 'machinery'} onClick={() => onNavigate('machinery')} />
                <NavItem icon={Package} label="Recursos" active={currentPage === 'inventory'} onClick={() => onNavigate('inventory')} />
